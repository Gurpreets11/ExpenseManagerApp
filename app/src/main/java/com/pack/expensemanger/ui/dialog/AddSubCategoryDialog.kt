package com.pack.expensemanger.ui.dialog

import com.pack.expensemanger.R
import com.pack.expensemanger.data.storage.entity.CategoryEntity
import com.pack.expensemanger.data.storage.entity.SubCategoryEntity


import android.app.Dialog
import android.os.Bundle
import android.widget.EditText
import android.widget.Spinner
import android.widget.ArrayAdapter
import androidx.appcompat.app.AlertDialog
import androidx.fragment.app.DialogFragment


class AddSubCategoryDialog(
    private val categories: List<CategoryEntity>,
    private val onSubCategoryAdded: (SubCategoryEntity) -> Unit
) : DialogFragment() {

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val view = requireActivity().layoutInflater.inflate(R.layout.dialog_add_sub_category, null)
        val etSubCategoryName = view.findViewById<EditText>(R.id.etSubCategoryName)
        val spinnerCategories = view.findViewById<Spinner>(R.id.spinnerCategories)

        // Populate spinner with categories
        val categoryNames = categories.map { it.name }
        val adapter =
            ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, categoryNames)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinnerCategories.adapter = adapter

        return AlertDialog.Builder(requireContext())
            .setTitle("Add SubCategory")
            .setView(view)
            .setPositiveButton("Add") { _, _ ->
                val name = etSubCategoryName.text.toString()
                val selectedCategoryIndex = spinnerCategories.selectedItemPosition
                if (name.isNotEmpty() && selectedCategoryIndex >= 0) {
                    val categoryId = categories[selectedCategoryIndex].id
                    val subCategory = SubCategoryEntity(name = name, categoryId = categoryId)
                    onSubCategoryAdded(subCategory)
                }
            }
            .setNegativeButton("Cancel", null)
            .create()
    }
}
